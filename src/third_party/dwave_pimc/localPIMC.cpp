...
void localPIMC::set_scaled_params(std::vector<std::vector<double> > invTempJ0,
                                  std::vector<double> invTempH0,
                                  double invTempGamma0,
                                  double invTempJchain0) {
    invTempJ.swap(invTempJ0);
    invTempH.swap(invTempH0);
    invTempGamma = invTempGamma0;
    if (!std::isnan(invTempJchain0)) {
        invTempJchain = invTempJchain0;
    }
}

double localPIMC::scaled_energy(const std::vector<int> &spins) const {
    assert(static_cast<int>(spins.size()) == numVar);
    double energy = 0.0;
    for (int i = 0; i < numVar; ++i) {
        energy += invTempH[i] * spins[i];
    }
    for (int i = 0; i < numVar; ++i) {
        for (std::size_t k = 0; k < adjMat[i].size(); ++k) {
            int j = adjMat[i][k];
            energy += 0.5 * invTempJ[i][k] * spins[i] * spins[j];
        }
    }
    return energy;
}

void localPIMC::set_seed(unsigned int seed) {
    initPRNG(seed);
}
