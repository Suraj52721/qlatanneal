...
class localPIMC {
   ...
   public:
    ...
    int size() const { return numVar; }
    const std::vector<int> &current_spins() const { return firstSlice; }
    void set_scaled_params(std::vector<std::vector<double> > invTempJ0,
                           std::vector<double> invTempH0,
                           double invTempGamma0,
                           double invTempJchain0 = std::numeric_limits<double>::quiet_NaN());
    double scaled_energy(const std::vector<int> &spins) const;
    double scaled_energy() const { return scaled_energy(firstSlice); }
