...
It provides five annealing engines, spanning the spectrum from purely
...
SQA-Chi & \code{sqa\_chi} & Worldline QMC with a $\chiB$-measured schedule & \ref{ch:optimal}\\
\bottomrule
\end{tabular}
\end{center}

All five engines share a single high-level Python entry point,
\code{solve()}, and a common C++17 core exposed through pybind11 bindings.
On top of the engines, \qv{} implements two theoretically grounded adaptive
schedules (Chapter~\ref{ch:optimal}), both built on a measured
susceptibility $\chiB$ as a stand-in for the (unknown) spectral gap: the
\emph{optimal adaptive $\Jp$ schedule}, derived directly from the local
adiabaticity condition of Roland and Cerf, and the standalone
\code{sqa\_chi} method, which measures the susceptibility of the worldline
magnetization instead. Both slow the annealing trajectory down where the
simulated system is close to its quantum critical point and accelerate it
elsewhere.
...
\node[layer,below=of b] (c)
  {\textbf{C++17 core}\quad \code{Annealer} \,\textbullet\, \code{ReplicaAnnealer} \,\textbullet\, \code{SQAAnnealer} \,\textbullet\, \code{SQAChiAnnealer} \,\textbullet\, \code{SQAParallelTemperingAnnealer} \,\textbullet\, \code{CTPIMCAnnealer}};
...
\item[Part III --- Adaptive schedules.] The local-adiabaticity derivation,
the bond susceptibility $\chiB$, budget calibration, the inverse-CDF
trajectory, and how to diagnose a run --- plus the standalone
\code{sqa\_chi} method, which builds its schedule from the susceptibility
of the worldline magnetization instead.
