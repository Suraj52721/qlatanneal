...
\code{SQAChiAnnealer} & \code{sqa\_chi\_annealer.hpp}\\
...
\code{sqa\_chi} (Section~\ref{sec:sqa-chi}) is a separate class --- note it
takes no \code{schedule} argument, since \code{run\_chi} builds its
trajectory entirely from \code{beta}/\code{gamma\_start}/\code{gamma\_end}:

\begin{cppcode}
qanneal::SQAChiAnnealer chi(ising, /*trotter_slices=*/32, /*replicas=*/4);
auto cres = chi.run_chi(/*beta=*/1.0,
                        /*gamma_start=*/5.0, /*gamma_end=*/0.01,
                        /*num_steps=*/150, /*sweeps_per_step=*/20,
                        /*scan_points=*/16, /*scan_sweeps=*/30,
                        /*scan_burn=*/10);
\end{cppcode}
